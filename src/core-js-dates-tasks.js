...
function getNextFriday(date) {
  ...
  if (index < 5) {
    NextFriday.setDate(date.getDate() + 5 - index);
  } else if (index === 5) {
    NextFriday.setDate(date.getDate() + 7);
  } else {
    NextFriday.setDate(date.getDate() + 6);
  }
  return NextFriday;
}
 ...
 * 86 400 000  milliseconds in the day
 * +1 needed to include both the start and end date.
 */
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const count = new Date(dateEnd) - new Date(dateStart);
  const millisec = 86400000;
  return Math.ceil(count / millisec) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    currentDate - startDate >= 0 &&
    endDate - currentDate >= 0 &&
    endDate - currentDate <= endDate - startDate
  );
}
...
function formatDate(date) {
  const date1 = new Date(date);
  const hours = date1.getUTCHours() % 12 || 12;
  const temp = date1.getUTCHours() >= 12 ? 'PM' : 'AM';
  const minute = date1.getUTCMinutes().toString().padStart(2, '0');
  const second = date1.getUTCSeconds().toString().padStart(2, '0');
  return `${date1.getUTCMonth() + 1}/${date1.getUTCDate()}/${date1.getUTCFullYear()}, ${hours}:${minute}:${second} ${temp}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const tempDate = new Date(date.setDate(date.getDate() + 4 - dayNum));
  const daysOnPeriod = getCountDaysOnPeriod(yearStart, tempDate);
  return Math.ceil(daysOnPeriod / 7);
}
...
function getNextFridayThe13th(date) {
  let newDate = getNextFriday(date);
  while (newDate.getDate() !== 13) {
    newDate = getNextFriday(newDate);
  }
  return newDate;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= end) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      if (workDate > end) break;
      datesWork.push(new Date(workDate));
    }
    currentDate = new Date(
      currentDate.setDate(currentDate.getDate() + countWorkDays + countOffDays)
    );
  }
  return datesWork.map(formatDate1);
}
